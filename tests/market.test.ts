...
test('parses a source-local clock with an explicit fixed timezone offset', () => { const clockConfig = {...config,timestampSelector:'#clock',timestampAttribute:'text' as const,timeZoneOffset:'+03:00'}; const source = '<span id="clock">۱۲:۵۹:۰۰</span><b class="buy">۹۸٬۵۰۰</b><b class="sell">۹۹٬۱۰۰</b>'; const [quote] = parseSource(source,clockConfig,new Date('2026-09-13T10:00:00Z')); assert.equal(quote.observedAt,'2026-09-13T09:59:00.000Z'); });
test('strips a declared text suffix without inventing prices', () => {
  const suffixConfig = {
    verified: true as const,
    timestampSelector: 'time',
    timestampAttribute: 'datetime' as const,
    assets: [{
      symbol: 'XAU_USD' as const,
      buySelector: '.buy',
      sellSelector: '.sell',
      textSuffix: 'USD' as const,
      currency: 'USD' as const,
      unit: 'اونس تروا',
    }],
  };
  const source = '<time datetime="2026-09-13T09:59:00Z"></time><b class="buy">۴٬۲۸۹ USD</b><b class="sell">۴٬۲۹۱ USD</b>';
  const [quote] = parseSource(source, suffixConfig, now);
  assert.equal(quote.symbol, 'XAU_USD');
  assert.equal(quote.buy, '4289');
