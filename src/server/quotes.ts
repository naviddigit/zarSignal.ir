...
import { demoQuotes, instruments, isStale, type Quote, type Snapshot } from '@/lib/market';
...
export async function getSnapshot(): Promise<Snapshot> {
  if (process.env.MARKET_MODE === 'demo' || (!process.env.MARKET_MODE && process.env.NODE_ENV !== 'production')) return { mode: 'demo', status: 'demo', quotes: demoQuotes };
