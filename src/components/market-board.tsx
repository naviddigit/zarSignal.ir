...
export function AssetMark({ symbol, category }: { symbol: Symbol; category: string }) {
  ...
  return <span className={`market-asset-mark ${category}`} aria-hidden="true"><Icon size={22}/></span>;
}
...
export function MarketBoard({ initial }: { initial: Snapshot }) {
  ...
  function toggle(symbol: Symbol) {
    ...
    setFavorites(next);
    try { localStorage.setItem('zarsignal-favorites', JSON.stringify(next)); } catch {}
  }
  ...
  return <section id="markets" className="panel market-panel">
    <header className="market-heading">
      <div><span className="eyebrow">MARKET WATCH</span><h2>نبض بازار</h2><p>قیمت خرید و فروش، واحد، منبع و زمان دریافت در یک نمای شفاف.</p></div>
      <div className={`feed-state ${live ? 'is-live' : 'is-offline'}`}><span/><strong>{live ? 'منبع داده متصل' : 'منبع زنده متصل نیست'}</strong><small><RefreshCw className={refreshing ? 'spin' : ''} size={12}/>{latestFetch ? <RelativeTime value={latestFetch} prefix="آخرین دریافت: "/> : `بررسی هر ${new Intl.NumberFormat('fa-IR').format(snapshot.pollSeconds ?? 60)} ثانیه`}</small></div>
    </header>
    <div className="market-toolbar">
      <div className="tabs">{[['all','همه'],['gold','طلا'],['silver','نقره'],['currency','ارز'],['favorites','منتخب']].map(([key,label]) => <button key={key} className={category === key ? 'selected' : ''} onClick={() => setCategory(key)} aria-pressed={category === key}>{label}</button>)}</div>
      <label className="search"><Search size={17}/><input aria-label="جست‌وجوی بازار" placeholder="نام یا نماد دارایی" value={query} onChange={event => setQuery(event.target.value)}/></label>
    </div>
    <div className={`source-disclosure ${live && !error ? 'is-live' : ''}`} role="status"><strong>{error ? 'به‌روزرسانی ناموفق بود' : live ? 'داده زنده ثبت شده' : 'قیمت نمایشی حذف شده است'}</strong><span>{live ? 'زمان دریافت هر ردیف زنده است؛ برای زمان دقیق نشانگر را نگه دارید.' : 'تا دریافت موفق از منبع معتبر، هیچ عددی به عنوان قیمت بازار نمایش داده نمی‌شود.'}</span></div>
    <div className="table-scroll"><table className="market-table"><thead><tr><th>دارایی</th><th>فروش</th><th>خرید</th><th>منبع و زمان</th><th>تحلیل</th><th aria-label="علاقه‌مندی"/></tr></thead><tbody>{assets.map(asset => {
      ...
      return <tr className={updated.includes(asset.symbol) ? 'quote-updated' : ''} key={asset.symbol}>
        ...
        <td>{quote ? <div className="quote-source">{quote.sourceUrl ? <a href={quote.sourceUrl} target="_blank" rel="noreferrer">{quote.source}</a> : <strong>{quote.source}</strong>}<small><Clock3 size={12}/><RelativeTime value={quote.fetchedAt}/></small></div> : <span className="no-source">متصل نیست</span>}</td>
        <td><span className="status-pill">در انتظار فرمول</span></td>
        <td><button className="icon-button" onClick={() => toggle(asset.symbol)} aria-label={`نشان‌کردن ${asset.name}`} aria-pressed={favorites.includes(asset.symbol)}><Star size={19} fill={favorites.includes(asset.symbol) ? 'currentColor' : 'none'}/></button></td>
      </tr>;
    })}</tbody></table>{!assets.length && <p className="empty-state">دارایی‌ای با این عبارت پیدا نشد.</p>}</div>
