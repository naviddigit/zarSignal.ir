  ...
  return <section id="markets" className="panel market-panel">
    <header className="market-heading">
      ...
      <div className={`feed-state ${fresh ? 'is-live' : stale ? 'is-stale' : 'is-offline'}`}>
        ...
        <small><RefreshCw className={refreshing ? 'spin' : ''} size={12}/>{latestFetch ? <RelativeTime value={latestFetch} prefix="آخرین دریافت: "/> : `بررسی هر ${new Intl.NumberFormat('fa-IR').format(snapshot.pollSeconds ?? 60)} ثانیه`}</small>
      </div>
    </header>
