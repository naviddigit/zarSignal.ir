...
export function Checkbox({ name, label, description, defaultChecked, disabled, className = '' }: CheckboxProps) {
  return <label className={`ds-checkbox ${className}`.trim()}>
    <CheckboxPrimitive.Root className="ds-checkbox__control" name={name} value="on" defaultChecked={defaultChecked} disabled={disabled}>
