...
export default async function AdminData({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return <><header className="admin-title"><div><span className="eyebrow">MARKET DATA</span><h1>منبع و دریافت داده</h1><p>آدرس منبع، فاصله دریافت، نتیجه آخرین اجرا و زمان هر قیمت را از همین‌جا کنترل کنید.</p></div><span className={`admin-badge ${data.database}`}>{data.database === 'connected' ? 'دیتابیس متصل' : 'دیتابیس در دسترس نیست'}</span></header>
  ...
  <article className="admin-card wide source-control"><div className="admin-card-heading"><div><span className="eyebrow">CRAWLER SOURCE</span><h2>HamRate</h2></div><a href={source.url} target="_blank" rel="noreferrer">مشاهده منبع <ExternalLink size={14}/></a></div>
   ...
   <div className="source-facts"><span><b>روش:</b> خواندن HTML عمومی با selectorهای اعتبارسنجی‌شده</span><span><b>حداقل فاصله:</b> ۱۸۰ ثانیه</span><span><b>نمادها:</b> ۶ بازار</span></div>
   <form action={runMarketSourceNow}><PendingButton className="button source-run" pendingText="در حال دریافت…" disabled={!source.enabled || data.database !== 'connected'}><Play size={15}/> دریافت و ثبت همین حالا</PendingButton></form>
  </article>
