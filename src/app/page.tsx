...
export default async function Home() {
  ...
  return <main id="main" className="shell home-market-first"><div className="topline"><span><span className="status-dot"/> دیده‌بان هوشمند بازار ایران</span><span>{snapshot.status === 'ok' ? 'قیمت دریافت‌شده · محاسبه شفاف' : snapshot.status === 'stale' ? 'آخرین داده ثبت‌شده · قیمت‌ها قدیمی‌اند' : 'در انتظار دریافت داده معتبر'}</span></div>
